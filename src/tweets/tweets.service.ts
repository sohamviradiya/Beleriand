...
export class TweetsService {
	...
	async findOne(id: string): Promise<Tweet> {
		return await this.tweetModel.findById(id).populate('user').populate('likes').exec();
	}
	...
	async verifyTweetOwner(id: string, user: string): Promise<boolean> {
		return String((await this.tweetModel.findById(id).select('user').exec()).user) === user;
	}
	...
	async dislike(id: string, userId: string): Promise<Tweet> {
		return await this.tweetModel.findByIdAndUpdate(id, { $pull: { likes: userId } }, { new: true }).exec();
	}
}
