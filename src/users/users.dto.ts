...
export default class InitUserDto {
	...
	@Matches(/(?=.*[a-z])/g, { message: 'Password must contain at least one lowercase letter' })
	@Matches(/(?=.*[A-Z])/g, { message: 'Password must contain at least one uppercase letter' })
	@Matches(/(?=.*[0-9])/g, { message: 'Password must contain at least one number' })
	@Matches(/(?=.*[!@#$%^&*~`?/])/g, { message: 'Password must contain at least one special character' })
